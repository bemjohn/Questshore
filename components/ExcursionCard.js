...
export default function ExcursionCard({ excursion, destinationPort, destinationId }) {
  const imageUrl = excursion.image
    ? urlFor(excursion.image).width(600).height(400).url()
    : `https://placehold.co/600x400/0c4a6e/ffffff?text=${encodeURIComponent((excursion.name || '').substring(0, 20))}`;

  const adultPrice = excursion.adultPrice ?? excursion.pricing?.adult;
  const childPrice = excursion.childPrice ?? excursion.pricing?.child;
        ...
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-sm">
          {adultPrice != null && (
            <div className="text-xs font-bold text-sky-800">
              Adult <span className="text-base">${adultPrice}</span>
            </div>
          )}
          {childPrice != null && (
            <div className="text-xs font-bold text-sky-600">
              Child <span className="text-base">${childPrice}</span>
            </div>
          )}
        </div>
      </div>
